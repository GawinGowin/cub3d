...
/*   Updated: 2024/10/21 08:08:33 by saraki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_dda	dda(t_player *player, char **map)
{
	...
	pos_int[0] = (int)player->pos_x;
	pos_int[1] = (int)player->pos_y;
	side_dist = get_side_distination(player, ret.ray_distination,
			ret.delta_dist, pos_int);
	ray_cast(side_dist, pos_int, map, &ret);
	return (ret);
}
			...
			int pos_int[2])
{
	...
	{
		ret.step[0] = -1;
		ret.dist[0] = (player->pos_x - pos_int[0]) * delta_dist[0];
	}
	else
	{
		ret.step[0] = 1;
		ret.dist[0] = (pos_int[0] + 1.0 - player->pos_x) * delta_dist[0];
	}
	if (ray_distination[1] < 0)
	{
		ret.step[1] = -1;
		ret.dist[1] = (player->pos_y - pos_int[1]) * delta_dist[1];
	}
	else
	{
		ret.step[1] = 1;
		ret.dist[1] = (pos_int[1] + 1.0 - player->pos_y) * delta_dist[1];
	}
	return (ret);
}
				...
				t_dda *ret)
{
	...
	while (ret->is_hit == 0)
	{
		if (ret->is_hit == 0 && distance[0] < distance[1])
		{
			...
			pos_int[0] += side_dist.step[0];
			ret->side = 0;
		}
		else if (ret->is_hit == 0 && distance[0] >= distance[1])
		{
			distance[1] += ret->delta_dist[1];
			pos_int[1] += side_dist.step[1];
			ret->side = 1;
		}
		if (map[pos_int[0]][pos_int[1]] == WALL)
			ret->is_hit = 1;
	}
	get_distance_and_hitblock(pos_int, distance, ret);
}
...
static void	get_distance_and_hitblock(
				...
				t_dda *ret)
{
	ret->hit_block[0] = pos_int[0];
	ret->hit_block[1] = pos_int[1];
	if (ret->side == 0)
		ret->distance = dist[0];
	else
		ret->distance = dist[1];
